"""
Ablation study: RoPE vs NoPE (no positional embedding).

RoPE:  Q and K are rotated by position-dependent angles before attention.
NoPE:  Q and K are left unrotated — the model sees no positional information.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-dir",      type=str,   default="output/rope_ablation")
    return p.parse_args()
...
def run_condition(base_cfg: Config, use_rope: bool, lr: float,
                  args: argparse.Namespace, out_dir: Path) -> dict:
    theta = base_cfg.model.theta if use_rope else None
    label = f"{'rope' if use_rope else 'nope'}_lr{lr:.2e}"
    ...
    cfg = replace(
        base_cfg,
        model=replace(base_cfg.model, theta=theta),
    ...
    return {"label": label, "use_rope": use_rope, "lr": lr,
            "diverged": diverged, "train_curve": train_curve, "val_curve": val_curve}


def plot_comparison(result_rope: dict, result_nope: dict, title: str, out_path: Path) -> None:
    ...
    for r, color, ls in [
        ...
        lbl = ("RoPE" if r["use_rope"] else "NoPE") + suffix
...
def main() -> None:
    ...
    for lr, lr_label in [(args.optimal_lr, "optimal"), (args.lower_lr, "lower")]:
        ...
        for use_rope in (True, False):
            name = "RoPE" if use_rope else "NoPE"
            print(f"\n--- {name}, lr={lr:.2e} ---")
            r = run_condition(default_config, use_rope, lr, args, out_dir)
            results.append(r)
